...
namespace SOFTWAREPROJECT_2024.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult AdminSıgnIn(Admin model)
        {
            ...
            //burada session alıyorum ki yukarıdan loginsiz bir şekilde home/adminpanel diyerek girlmesini eneglliyice adminpanel sayfasına fevzig 
        ...
        public IActionResult DeleteMember(int MemberId)
        {
            ...
            var borrowinfo = _context.Borrowings.Where(y=>y.MemberId == MemberId).ToList();//tüm rezervasyonları getir
            if (memberdelete != null)
            {
                _context.Borrowings.RemoveRange(borrowinfo);//birden fazla varsa sil
                _context.Members.Remove(memberdelete);
            // Eğer giriş yapan kullanıcı bu üye ise, çıkış yapalım
        ...
        public IActionResult DeleteBook(int BookId)
        {
            ...
            var borrowsinfo = _context.Borrowings.Where(y=>y.BookId == BookId).ToList();

            if (information != null)
            {
                _context.Borrowings.RemoveRange(borrowsinfo);// diyerek o kitapla ilgili rezervasyonlarıda silcez
